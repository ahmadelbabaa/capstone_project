...
try:
    from .data import MATCH_ID_MAPPING_FILE, RAW_DATA_DIR, PROCESSED_DATA_DIR, PROJECT_ROOT, USL_TRACKING_DIR, USL_DATA_DIR
...
def load_events_data():
    ...
    print(f"Loading events from JSON: {events_file}")
...
def load_pitch_dimensions(skillcorner_match_id):
    ...
    """
    try:
        match_file = RAW_DATA_DIR / 'metadata_j1_2024' / f'{skillcorner_match_id}_metadata.json'
...
def main(max_workers=None, use_parallel=True, subsample_seconds=0.2, priority_events=['Pass', 'Carry', 'Shot'], force_reprocess=False, batch_size=20, skip_coordinates=False, preserve_all_frames=False):
    ...
    # Get all tracking files (JSON format)
    tracking_pattern = str(RAW_DATA_DIR / 'tracking_j1_2024' / '*_tracking_extrapolated.jsonl')
    all_tracking_files = glob.glob(tracking_pattern)
    
    if not all_tracking_files:
        print(f"[ERROR] No tracking files found with pattern: {tracking_pattern}")
        return
    
    print(f"Found {len(all_tracking_files)} total tracking JSON files")
